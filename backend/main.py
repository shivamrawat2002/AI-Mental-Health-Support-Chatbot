...
async def ask(query: Query):
    thread_id = "web_user"  # later: user_id / session_id
...
async def whatsapp_ask(
    ...
    print("RAW WHATSAPP BODY:", repr(Body))

    user_text = Body.strip() if Body else ""
    thread_id = From  # ✅ unique per WhatsApp user
    ...
    if not final_response:
        final_response = (
            "I'm here to support you, but I couldn't generate a response just now."
        )
